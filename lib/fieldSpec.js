...
function lookupFunc(table) {
    if (typeof table === 'undefined') {
        return (v) => v;
    }
    console.error('WARNING: Table lookups not yet implemented');
    let lookupFn = (v) => v;
    return (v) => v && lookupFn(v.toString());
}
function fieldValueFunction(fieldSpec, headers) {
    const components = fastFieldComponents(fieldSpec);
    return (rowData) => lookupFunc(components.lookup)(typeFunc(components.type)(lengthFunc(components.length)(fieldFunc(components.field, headers)(rowData))));
}

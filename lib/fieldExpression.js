...
function expressionValueFunction(expression, headers, tableCache, staticVars = {}) {
    ...
    if (typeof exprs[0] === 'string' && exprs[0][0] === '$') {
        ...
        if (exprs[0] !== '$' || typeof exprs[1] !== 'function' || exprs.slice(2).reduce((a, c) => (a || typeof c === 'function'), false)) {
    ...
    if (typeof exprs[0] === 'string' && exprs[0][0] === '!') {
        ...
        return (rowData) => {
            // console.info(exprFn(rowData))
            return fieldSpec_1.valueFunction(exprFn(rowData).slice(1), headers, tableCache)(rowData);
        };
    }
    else {
